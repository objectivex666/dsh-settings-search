...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const REPO_URL = 'https://github.com/objectivex666/dsh-settings-search'
    const ISSUES_URL = 'https://github.com/objectivex666/dsh-settings-search/issues/new/choose'
    const zh = {
      title: '设置',
      placeholder: '搜索设置…',
      searchBtn: '搜索',
      ...
      stylePage: '弹出搜索页',
      openSearch: '打开搜索',
      shortcutTitle: 'Ctrl+F / ⌘F 快捷键',
      shortcutHint: '在设置面板内按 Ctrl+F 或 ⌘F 聚焦搜索框；关闭后保留浏览器默认查找。',
      indexTitle: '设置索引',
      indexDesc: '索引保存在本机浏览器；只在首次安装或更新其它插件后重建，也可以手动重建。',
      indexRebuild: '手动重建索引',
      indexRebuilding: '正在重建索引…',
      indexDone: '索引已重建 ✓',
      warming: '正在导入各分区选项…',
      empty: '没有匹配的设置',
      pages: '设置页',
      ...
      aiSearch: 'AI 联想搜索',
      aiDesc: '没有直接匹配时，用你配置的模型（OpenAI 兼容、Anthropic 或本机 Ollama）理解目标并推荐设置项。',
      aiPrivacy: 'AI 请求会发送到你配置的模型服务；API Key 仅保存在本机浏览器。',
      aiOn: '已开启',
      aiOff: '已关闭',
      aiThinking: '正在用 AI 理解搜索意图…',
      aiUnavailable: 'AI 联想暂不可用',
      aiNeedConfig: '请先在「设置搜索」页配置 Base URL、模型名与 API Key（Ollama 无需密钥）。',
      aiProvider: '接口类型',
      aiOpenAI: 'OpenAI 兼容',
      ...
      aiModel: '模型名',
      aiModelHint: '从接口返回的模型中选择，或手动输入完整模型名。',
      aiModelsLoad: '获取模型',
      aiModelsLoading: '获取中…',
      aiModelsLoaded: '已获取 {count} 个模型，可直接选择。',
      aiModelsEmpty: '服务未返回模型，可手动输入。',
      aiModelsFail: '获取模型失败：{hint}{detail}',
      aiModelsNeedConfig: '先填写有效接口地址；云端接口还需 API Key。',
      aiApiKey: 'API Key',
      ...
      aiOllamaErrNetwork: '无法连接 Ollama，请确认本机服务已启动、地址可达，且 OLLAMA_ORIGINS 允许当前 DSH 页面来源',
      aiOllamaErrForbidden: 'Ollama 拒绝访问，请检查 OLLAMA_ORIGINS 是否允许当前 DSH 页面来源',
      aiOllamaErrTimeout: 'Ollama 响应超时，请检查本机服务及模型加载状态',
      logTitle: '日志',
      ...
      wlFeature1Desc: '支持经典侧栏、窗口顶栏与弹出搜索页；输入关键词即可过滤设置。',
    ...
    const en = {
      ...
      shortcutTitle: 'Ctrl+F / ⌘F shortcut',
      shortcutHint: 'Focus the search box with Ctrl+F or ⌘F inside the settings panel; turn it off to keep the browser default.',
      warming: 'Importing section options…',
      indexTitle: 'Settings index',
      indexDesc: 'The index is stored locally; it rebuilds only after another plugin is installed or updated. You can also rebuild it manually.',
      indexRebuild: 'Rebuild index',
      indexRebuilding: 'Rebuilding…',
      indexDone: 'Index rebuilt ✓',
      ...
      aiDesc: 'When no direct match exists, your own model (OpenAI-compatible, Anthropic, or local Ollama) understands your goal and suggests settings.',
      ...
      aiNeedConfig: 'Configure Base URL, model, and API key in the Settings Search panel first (Ollama needs no key).',
      aiProvider: 'API type',
      aiOpenAI: 'OpenAI-compatible',
      aiAnthropic: 'Anthropic',
      aiOllama: 'Ollama (local)',
      aiBaseUrl: 'Base URL',
      aiBaseUrlHint: 'Only http(s) URLs are allowed; remote hosts must use https to avoid exposing your API key.',
      aiModel: 'Model',
      aiModelHint: 'Choose a model returned by the API, or enter a complete model name.',
      aiModelsLoad: 'Get models',
      aiModelsLoading: 'Loading…',
      aiModelsLoaded: 'Loaded {count} models; choose one below.',
      aiModelsEmpty: 'The service returned no models; enter one manually.',
      aiModelsFail: 'Failed to load models: {hint}{detail}',
      aiModelsNeedConfig: 'Enter a valid base URL first; cloud APIs also need an API key.',
      aiApiKey: 'API Key',
      ...
      aiOllamaErrNetwork: 'Cannot connect to Ollama. Check that the local service is running, the URL is reachable, and OLLAMA_ORIGINS allows this DSH page origin',
      aiOllamaErrForbidden: 'Ollama denied access. Check that OLLAMA_ORIGINS allows this DSH page origin',
      aiOllamaErrTimeout: 'Ollama timed out. Check the local service and model loading status',
      logTitle: 'Logs',
      logDesc: 'Logs key plugin actions, searches, and AI requests to help troubleshoot.',
      logEmpty: 'No logs yet',
      logExport: 'Export logs',
      logIssues: 'Report issue',
      logCopy: 'Copy logs',
      logCopied: 'Copied ✓',
      ...
      wlFeature1Desc: 'Choose classic sidebar, top bar, or popup search page; typing filters settings.',
    ...
    const normalizeAiConfig = (cfg) => {
      const provider = ['anthropic', 'ollama'].includes(cfg.provider) ? cfg.provider : 'openai'
      return {
        provider,
        baseUrl: String(cfg.baseUrl ?? '').trim() || (provider === 'ollama' ? OLLAMA_BASE_URL : ''),
        model: String(cfg.model ?? '').trim(),
        apiKey: String(cfg.apiKey ?? '').trim(),
      }
    }
    const aiConfigReady = (cfg) => {
      const { provider, baseUrl, model, apiKey } = normalizeAiConfig(cfg)
      return Boolean(baseUrl && model && (provider === 'ollama' || apiKey))
    }
    ...
    const aiConfig = () => {
      ...
      return normalizeAiConfig({
        provider: (storage?.getItem(AI_PROVIDER_KEY) ?? 'openai').trim(),
        baseUrl: storage?.getItem(AI_BASE_URL_KEY),
        model: storage?.getItem(AI_MODEL_KEY),
        apiKey: readApiKey(),
      })
    }
    const setAiConfig = (cfg) => {
      ...
      writeApiKey(normalized.apiKey)
      if (typeof localStorage === 'undefined') return
      localStorage.setItem(AI_PROVIDER_KEY, normalized.provider)
      localStorage.setItem(AI_BASE_URL_KEY, normalized.baseUrl)
      localStorage.setItem(AI_MODEL_KEY, normalized.model)
    ...
    const SEARCH_SHORTCUT_KEY = 'dsh-settings-search:shortcut-enabled'
    const SEARCH_SHORTCUT_EVENT = 'dsh-settings-search:shortcut-change'
    const INDEX_STORAGE_KEY = 'dsh-settings-search:index:v1'
    const INDEX_INVALID_EVENT = 'dsh-settings-search:index-invalid'
    const INDEX_REBUILD_EVENT = 'dsh-settings-search:index-rebuild'
    const INDEX_REBUILD_DONE_EVENT = 'dsh-settings-search:index-rebuild-done'
    const SEARCH_STYLES = ['sidebar', 'top', 'page']
    ...
    const searchShortcutEnabled = () => {
      ...
      return localStorage.getItem(SEARCH_SHORTCUT_KEY) !== '0'
    }
    const setSearchShortcutEnabled = (enabled) => {
      if (typeof localStorage === 'undefined') return
      localStorage.setItem(SEARCH_SHORTCUT_KEY, enabled ? '1' : '0')
      window.dispatchEvent(new CustomEvent(SEARCH_SHORTCUT_EVENT, { detail: enabled }))
    }
    const welcomed = () => {
      if (typeof localStorage === 'undefined') return true
    ...
    const ollamaChatUrl = (baseUrl) => {
      const safe = safeBaseUrl(baseUrl)
      if (!safe) return ''
      if (/\/api\/chat$/i.test(safe)) return safe
      if (/\/api$/i.test(safe)) return `${safe}/chat`
      // Accept the base URL used by Ollama's OpenAI-compatible endpoint too.
      return `${safe.replace(/\/v1(?:\/chat\/completions)?$/i, '')}/api/chat`
    }
    const ollamaModelsUrl = (baseUrl) => {
      const safe = safeBaseUrl(baseUrl)
      if (!safe) return ''
      const root = safe.replace(/\/v1(?:\/models|\/chat\/completions)?$/i, '').replace(/\/api(?:\/tags|\/chat)?$/i, '')
      return `${root}/api/tags`
    }
    const chatModelsUrl = (baseUrl) => {
      const safe = safeBaseUrl(baseUrl)
      if (!safe) return ''
      const root = safe.replace(/\/(?:models|chat\/completions)$/i, '')
      return /\/v1$/i.test(root) ? `${root}/models` : `${root}/v1/models`
    }
    const anthropicModelsUrl = (baseUrl) => {
      const safe = safeBaseUrl(baseUrl)
      if (!safe) return ''
      const root = safe.replace(/\/v1\/(?:models|messages)$/i, '')
      return `${root}/v1/models`
    }
    const modelsUrl = (provider, baseUrl) => provider === 'ollama'
      ? ollamaModelsUrl(baseUrl)
      : provider === 'anthropic' ? anthropicModelsUrl(baseUrl) : chatModelsUrl(baseUrl)
    const extractAiModels = (provider, data) => {
    ...
    const fetchAiModels = async (cfg, signal) => {
      cfg = normalizeAiConfig(cfg)
      const { provider, baseUrl, apiKey } = cfg
      const url = modelsUrl(provider, baseUrl)
      if (!url) return { ok: false, code: 'unsafe-url', models: [], detail: '' }
      if (provider !== 'ollama' && !apiKey) return { ok: false, code: 'need-config', models: [], detail: '' }
      ...
      const deadline = new Promise((_, reject) => {
        timer = setTimeout(() => {
          const error = new Error('AI model list timed out')
      ...
      try {
        const models = await Promise.race([
          (async () => {
            ...
            if (data?.error) {
              const error = new Error('Unexpected model list response')
              error.name = 'AI_BAD_RESPONSE'
              throw error
            }
            return extractAiModels(provider, data)
          })(),
          deadline,
        ])
        return { ok: true, models, code: models.length === 0 ? 'empty' : 'ok', detail: '' }
      } catch (error) {
        if (error.name === 'AbortError') return { ok: false, code: 'cancelled', models: [], detail: '' }
        if (error.name === 'AI_TIMEOUT') return { ok: false, code: 'timeout', models: [], detail: '' }
        const code = error.status ? testHttpCode(error.status) : error.name === 'AI_BAD_RESPONSE' ? 'bad-response' : 'network'
        return { ok: false, code, models: [], detail: String(error?.message ?? error).slice(0, 160) }
      } finally {
        clearTimeout(timer)
        signal?.removeEventListener('abort', abort)
      }
    }
    const aiRequest = (cfg, userContent, testing = false) => {
      const { provider, baseUrl, model, apiKey } = normalizeAiConfig(cfg)
      ...
      if (provider === 'anthropic') {
        return {
          ...
          body: { model, messages, max_tokens: testing ? 1 : 1024, ...(testing ? {} : { system: AI_PROMPT, temperature: 0 }) },
        }
      }
      if (!testing) messages.unshift({ role: 'system', content: AI_PROMPT })
      if (provider === 'ollama') {
        return {
          url: ollamaChatUrl(baseUrl),
          headers,
          body: {
            model,
            messages,
            stream: false,
            ...(testing ? {} : { format: 'json' }),
            options: { temperature: 0, num_predict: testing ? 1 : 1024 },
          },
        }
      }
      return {
        ...
        body: { model, messages, temperature: 0, ...(testing ? { max_tokens: 1 } : {}) },
      }
    }
    const aiResponseContent = (provider, data) => {
      if (provider === 'ollama') return data?.message?.content
      if (provider === 'anthropic') {
        if (!Array.isArray(data?.content) || data.content.length === 0) return undefined
        return data.content.filter((block) => block?.type === 'text').map((block) => block.text).join('')
      }
      return data?.choices?.[0]?.message?.content
    }
    const fetchAiData = async (request, provider, signal) => {
      const controller = new AbortController()
      const abort = () => controller.abort()
      if (signal?.aborted) abort()
      else signal?.addEventListener('abort', abort, { once: true })
      ...
      const work = (async () => {
        ...
        if (data?.error || typeof aiResponseContent(provider, data) !== 'string') {
          const error = new Error('Unexpected AI response')
          error.name = 'AI_BAD_RESPONSE'
          throw error
        }
        return data
    ...
    const requestUserAi = async (query, labels, signal) => {
      const cfg = aiConfig()
      if (!aiConfigReady(cfg)) return []
      const { provider, model } = cfg
      const request = aiRequest(cfg, `用户目标：${query}\n设置索引：\n${labels}`)
      if (!request.url) return []
      const startedAt = Date.now()
      log('info', 'AI 请求', { provider, model, query: String(query).slice(0, 50) })
      try {
        const data = await fetchAiData(request, provider, signal)
        const keywords = extractAiKeywords(aiResponseContent(provider, data))
    ...
    const requestAiKeywords = async (query, labels, signal) => {
      const { provider, baseUrl, model } = aiConfig()
      const cacheKey = JSON.stringify([provider, baseUrl, model, query.toLowerCase(), labels])
    ...
    const testAiService = async (cfg, signal) => {
      cfg = normalizeAiConfig(cfg)
      const { provider, model } = cfg
      if (!aiConfigReady(cfg)) return { ok: false, code: 'need-config', detail: '' }
      const request = aiRequest(cfg, 'ping', true)
      if (!request.url) return { ok: false, code: 'unsafe-url', detail: '' }
      const startedAt = Date.now()
      log('info', '测试 API 服务', { provider, model })
      try {
        await fetchAiData(request, provider, signal)
        log('info', '测试 API 成功', { duration: Date.now() - startedAt })
        return { ok: true }
      } catch (error) {
        if (error.name === 'AbortError') {
          log('warn', '测试 API 取消', { duration: Date.now() - startedAt })
          return { ok: false, code: 'cancelled', detail: '' }
        }
        ...
        return { ok: false, code, detail: reason }
      }
    }
    ...
    const CSS = `
      ...
      .sss-page-overlay { position: fixed; inset: 0; z-index: 1000; display: flex; align-items: flex-start; justify-content: center; padding: 72px 20px 32px; background: var(--dsw-alias-bg-mask, rgb(0 0 0 / 36%)); }
    ...
    const serializeHarvest = (harvest) => {
      const raw = {}
      for (const [slot, map] of harvest) {
        if (!(map instanceof Map) || map.size === 0) continue
        raw[slot] = Object.fromEntries([...map].filter(([, record]) => record && typeof record.title === 'string' && record.title !== ''))
      }
      return raw
    }
    const deserializeHarvest = (raw) => {
      const harvest = new Map()
      if (!raw || typeof raw !== 'object') return harvest
      for (const [slot, slotValue] of Object.entries(raw)) {
        if (!slotValue || typeof slotValue !== 'object') continue
        const map = new Map()
        for (const [key, record] of Object.entries(slotValue)) {
          if (!record || typeof record !== 'object' || typeof record.title !== 'string' || record.title === '') continue
          const inner = Array.isArray(record.inner)
            ? record.inner.filter((op) => op && typeof op.label === 'string').map((op) => ({ label: op.label, text: typeof op.text === 'string' ? op.text : '' }))
            : []
          map.set(key, {
            title: record.title,
            idx: Number.isFinite(record.idx) ? record.idx : 0,
            text: typeof record.text === 'string' ? record.text : '',
            inner,
          })
        }
        if (map.size > 0) harvest.set(slot, map)
      }
      return harvest
    }
    ...
    const readIndexCache = () => {
      ...
      try {
        const raw = JSON.parse(localStorage.getItem(INDEX_STORAGE_KEY) ?? 'null')
        if (!raw || raw.version !== 1 || typeof raw.fingerprint !== 'string' || raw.fingerprint === '') return null
        return {
          fingerprint: typeof raw.fingerprint === 'string' ? raw.fingerprint : null,
          harvest: deserializeHarvest(raw.harvest),
        }
      } catch {
        return null
      }
    ...
    const otherPluginFingerprint = (plugins) => {
      const rows = Array.isArray(plugins) ? plugins : []
      const isSelf = (row) => [
        row?.id, row?.name, row?.npm, row?.package, row?.packageName,
      ].some((value) => typeof value === 'string' && value.includes(NPM_NAME))
      return JSON.stringify(rows.filter((row) => row && !isSelf(row)).map((row) => [
        String(row.id ?? row.name ?? ''),
        String(row.npm ?? row.package ?? row.packageName ?? ''),
        String(row.version ?? ''),
      ].sort()).sort())
    }

    function createModel(ctx) {
      const initialCache = readIndexCache()
      const cachedFingerprint = initialCache?.fingerprint ?? null
      /** id → harvested visible title, per slot. */
      const harvest = initialCache?.harvest ?? new Map()
      ...
      const slotsFingerprint = () => otherPluginFingerprint([
        ...sortEntries(entriesOf('settings.plugin.item')).map((entry) => ({
          id: String(entry.options?.id ?? entry.options?.key ?? ''),
          name: optionLabel(entry.options?.label),
          locale: String(entry.options?.locale ?? ''),
        })),
        ...sortEntries(entriesOf('web-ui.plugin.item')).map((entry) => ({
          id: String(entry.options?.id ?? entry.options?.key ?? ''),
          name: optionLabel(entry.options?.label),
          locale: String(entry.options?.locale ?? ''),
        })),
      ...
      const resetHarvest = () => { harvest.clear(); warmed = false }

      const persistIndex = (nextFingerprint = fingerprint) => {
        if (typeof nextFingerprint !== 'string' || nextFingerprint === '' || harvest.size === 0) return false
        ...
        try {
          localStorage.setItem(INDEX_STORAGE_KEY, JSON.stringify({
            ...
            fingerprint,
            updatedAt: Date.now(),
            harvest: serializeHarvest(harvest),
          }))
          return true
        } catch {
          return false
      ...
      const indexFingerprintReady = () => typeof fingerprint === 'string' && fingerprint !== ''

      const applyIndexFingerprint = (nextFingerprint) => {
        if (typeof nextFingerprint !== 'string' || nextFingerprint === '') return false
        if (nextFingerprint === fingerprint) {
          if (nextFingerprint === cachedFingerprint) warmed = true
          return false
        }
        const wasPopulated = harvest.size > 0
        fingerprint = nextFingerprint
        resetHarvest()
        return wasPopulated
      }

      return { readSections, readDeep, runHarvest, persistIndex, applyIndexFingerprint, resetHarvest, slotsFingerprint, indexFingerprintReady }
    }

    /* ---------- DOM-level navigation into the settings shell ---------- */

    /**
     * Root element that hosts the settings panel's nav buttons. Derived from
     * the plugin's own mounted UI so an unrelated or plugin-owned dialog can
     * never become the click root; the first-dialog fallback only applies
     * when the plugin's anchors are not mounted.
     */
    const panelRoot = () => {
      if (typeof document === 'undefined') return null
      const anchor = document.querySelector('.sss-wrap, .sss-vr')
      if (anchor) return anchor.closest('[role="dialog"]') ?? document.body
    ...
    const clickNavButton = (wantText) => {
      ...
      const buttons = panelButtons().filter((b) => /_navCell/.test(String(b.className || '')))
    ...
    function apply(ctx) {
      ...
      const indexControls = { resetHarvest: model.resetHarvest, persistIndex: model.persistIndex }
      const navigator = createNavigator(ctx)
      let pluginManagerSyncToken = 0

      const syncPluginManagerIndex = async (face) => {
        if (face === null || !face.isLoopback) return
        const token = ++pluginManagerSyncToken
        try {
          const rows = await face.list()
          if (token !== pluginManagerSyncToken) return
          if (model.applyIndexFingerprint('manager:' + otherPluginFingerprint(rows))) {
            window.dispatchEvent(new CustomEvent(INDEX_INVALID_EVENT))
            log('info', '检测到其它插件变化，将重建设置索引', {})
          }
        } catch {
          // Keep the last trusted fingerprint when manager inspection fails.
        }
      }

      ctx.inject(['pluginManager'], (inner) => {
        const face = inner.pluginManager ?? null
        const sync = () => { void syncPluginManagerIndex(face) }
        sync()
        const unsubscribe = typeof face?.onChange === 'function' ? face.onChange(sync) : null
        return () => {
          pluginManagerSyncToken += 1
          if (typeof unsubscribe === 'function') unsubscribe()
        }
      }, 'settings-search: plugin manager index fingerprint')
      ...
      const readDshModelConfig = async () => {
        ...
        return {
          provider: providerId.toLowerCase() === 'ollama' || protocol === 'ollama'
            ? 'ollama'
      ...
      function SettingsHeaderSearch() {
        ...
        const [shortcutEnabled, setShortcutEnabled] = React.useState(searchShortcutEnabled)
        ...
        React.useEffect(() => {
          // If no plugin manager is present, the slot registrations are the
          // only available change signal. Waiting briefly avoids racing the
          // optional manager during startup.
          const fallback = setTimeout(() => {
            if (model.indexFingerprintReady()) return
            if (model.applyIndexFingerprint(model.slotsFingerprint())) {
              window.dispatchEvent(new CustomEvent(INDEX_INVALID_EVENT))
            }
          }, 3000)
          const onInvalid = () => { setWarming(false); setTick((value) => value + 1) }
          window.addEventListener(INDEX_INVALID_EVENT, onInvalid)
          return () => {
            clearTimeout(fallback)
            window.removeEventListener(INDEX_INVALID_EVENT, onInvalid)
          }
        }, [])
        ...
        // A style switch must not carry an open popup from the previous layout
        // into the new one. The host can keep this component mounted while the
        // settings choice changes, so reset transient view state explicitly.
        React.useEffect(() => {
          setOpen(false)
          setPageOpen(false)
          setHint(null)
        ...
        React.useEffect(() => {
          const sync = () => setShortcutEnabled(searchShortcutEnabled())
          window.addEventListener(SEARCH_SHORTCUT_EVENT, sync)
          return () => window.removeEventListener(SEARCH_SHORTCUT_EVENT, sync)
        }, [])
        ...
        const cfg = aiConfig()
        const aiConfigured = aiConfigReady(cfg)
        ...
        }, [aiReady, aiEnabled, q, cfg.provider, cfg.baseUrl, cfg.model, cfg.apiKey])
        ...
        React.useEffect(() => {
          ...
          const onKeyDown = (event) => {
            ...
            if (event.altKey || event.shiftKey || event.key !== 'f' && event.key !== 'F') return
      ...
      function UpdatePanel() {
        ...
        const [searchStyle, setSearchStyleState] = React.useState(readSearchStyle)
        const [shortcutOn, setShortcutOn] = React.useState(searchShortcutEnabled)
        ...
        const providerDrafts = React.useRef({})
        const testAbortRef = React.useRef(null)
        const modelsAbortRef = React.useRef(null)
        ...
        const [modelsState, setModelsState] = React.useState('idle') // idle | loading | ready | fail
        const [modelsMsg, setModelsMsg] = React.useState('')
        ...
        const [indexState, setIndexState] = React.useState('idle')
        const [warming, setWarming] = React.useState(false)
        ...
        React.useEffect(() => {
          const sync = () => setShortcutOn(searchShortcutEnabled())
          window.addEventListener(SEARCH_SHORTCUT_EVENT, sync)
          return () => window.removeEventListener(SEARCH_SHORTCUT_EVENT, sync)
        }, [])
        ...
        React.useEffect(() => () => modelsAbortRef.current?.abort(), [provider, baseUrl, apiKey])
        ...
        const changeProvider = (next) => {
          providerDrafts.current[provider] = { baseUrl, model }
          const draft = providerDrafts.current[next]
          setProvider(next)
          setBaseUrl(draft?.baseUrl ?? (next === 'ollama' ? OLLAMA_BASE_URL : ''))
        ...
        const modelsErrorHint = (code) => {
          if (code === 'need-config') return t('aiModelsNeedConfig')
          const cloudKeys = {
            auth: 'aiTestErrAuth',
          ...
          const ollamaKeys = { ...cloudKeys, network: 'aiOllamaErrNetwork', forbidden: 'aiOllamaErrForbidden', timeout: 'aiOllamaErrTimeout' }
          return t((provider === 'ollama' ? ollamaKeys : cloudKeys)[code] ?? 'aiTestErrHttp')
        }

        const loadModels = async () => {
          if (modelsState === 'loading') return
          const source = normalizeAiConfig({ provider, baseUrl, model, apiKey })
          if (!modelsUrl(source.provider, source.baseUrl) || (source.provider !== 'ollama' && !source.apiKey)) {
            setModelsState('fail')
            setModelsMsg(t('aiModelsFail').replace('{hint}', t('aiModelsNeedConfig')).replace('{detail}', ''))
            return
          }
          const controller = new AbortController()
          modelsAbortRef.current?.abort()
          ...
          setModelsMsg('')
          const startedAt = Date.now()
          log('info', '获取 AI 模型列表', { provider: source.provider })
          const result = await fetchAiModels(source, controller.signal)
          ...
          if (result.ok) {
            ...
            setModelsMsg(result.models.length === 0
              ? t('aiModelsEmpty')
              : t('aiModelsLoaded').replace('{count}', String(result.models.length)))
            log('info', '获取 AI 模型列表成功', { count: result.models.length, duration: Date.now() - startedAt })
          } else {
            setAiModels([])
            setModelsState('fail')
            const hint = modelsErrorHint(result.code)
            const detail = result.detail ? `（${result.detail}）` : ''
            setModelsMsg(t('aiModelsFail').replace('{hint}', hint).replace('{detail}', detail))
            log('error', '获取 AI 模型列表失败', { code: result.code, detail: result.detail, duration: Date.now() - startedAt })
          }
        }
        ...
        const rebuildIndex = async () => {
          if (indexState === 'loading' || warming) return
          setIndexState('loading')
          setWarming(true)
          indexControls.resetHarvest()
          try {
            const done = await warmAllSections(model, () => setWarming(true), () => setWarming(false))
            if (!done) throw new Error('index rebuild did not visit any sections')
            indexControls.persistIndex()
            setIndexState('done')
            window.dispatchEvent(new CustomEvent(INDEX_REBUILD_DONE_EVENT))
            setTimeout(() => setIndexState((state) => state === 'done' ? 'idle' : state), 2400)
          } catch (error) {
            setIndexState('idle')
            setWarming(false)
            log('error', '手动重建设置索引失败', { detail: String(error?.message ?? error).slice(0, 160) })
          }
        }

        const updateShortcutEnabled = (enabled) => {
          setSearchShortcutEnabled(enabled)
          setShortcutOn(enabled)
          log('info', enabled ? '启用搜索快捷键' : '禁用搜索快捷键', {})
        }
        ...
        const testService = async () => {
          ...
          if (!aiConfigReady({ provider, baseUrl, model, apiKey })) {
            setTestState('fail')
          ...
          const result = await testAiService({ provider, baseUrl, model, apiKey }, controller.signal)
          if (controller.signal.aborted) return
          ...
          if (result.ok) {
            ...
            setTestState('fail')
            const ollamaHintKey = provider === 'ollama' ? {
              network: 'aiOllamaErrNetwork',
              forbidden: 'aiOllamaErrForbidden',
              timeout: 'aiOllamaErrTimeout',
            }[result.code] : null
            const hintKey = ollamaHintKey ?? {
              auth: 'aiTestErrAuth',
              forbidden: 'aiTestErrForbidden',
              'not-found': 'aiTestErrNotFound',
              'rate-limit': 'aiTestErrRateLimit',
        ...
        return React.createElement('div', { className: 'sss-vr' + (open ? ' sss-vr-open' : '') }, [
          ...
          open ? React.createElement('div', { className: 'sss-vr-body' }, [
            ...
            React.createElement('div', { className: 'sss-vr-field' }, [
              React.createElement('div', { className: 'sss-vr-field-head' }, [
                React.createElement('span', { className: 'sss-vr-label' }, t('shortcutTitle')),
                React.createElement('input', {
                  className: 'sss-vr-check',
                  type: 'checkbox',
                  checked: shortcutOn,
                  onChange: (event) => updateShortcutEnabled(event.target.checked),
                }),
              ]),
              React.createElement('p', { className: 'sss-vr-hint' }, t('shortcutHint')),
            ]),
            React.createElement('div', { className: 'sss-vr-field' }, [
              React.createElement('div', { className: 'sss-vr-field-head' }, [
                React.createElement('span', { className: 'sss-vr-label' }, t('indexTitle')),
              ]),
              React.createElement('p', { className: 'sss-vr-hint' }, t('indexDesc')),
              React.createElement('div', { className: 'sss-vr-actions' }, [
                React.createElement('button', {
                  className: 'sss-vr-btn',
                  type: 'button',
                  disabled: indexState === 'loading' || warming,
                  onClick: () => { void rebuildIndex() },
                }, indexState === 'loading' || warming ? t('indexRebuilding') : t('indexRebuild')),
              ]),
              indexState === 'done'
                ? React.createElement('p', { className: 'sss-vr-note', style: { color: '#1e8e3e' } }, t('indexDone'))
                : null,
            ]),
            React.createElement('div', { className: 'sss-vr-field sss-vr-form' }, [
              ...
              React.createElement('select', {
                ...
                React.createElement('option', { value: 'ollama' }, t('aiOllama')),
              ]),
              React.createElement('div', { className: 'sss-vr-field-head' }, [
              ...
              React.createElement('input', {
                ...
                value: baseUrl,
                placeholder: provider === 'ollama' ? OLLAMA_BASE_URL
                ...
                'data-1p-ignore': 'true',
                'data-form-type': 'other',
                 onChange: (event) => { setBaseUrl(event.target.value); resetModelList() },
               }),
               React.createElement('p', { className: 'sss-vr-hint' }, t('aiBaseUrlHint')),
               React.createElement('div', { className: 'sss-vr-field-head' }, [
                 React.createElement('label', { className: 'sss-vr-label' }, t('aiModel')),
                 React.createElement('button', { className: 'sss-vr-btn', type: 'button', disabled: modelsState === 'loading', onClick: () => { void loadModels() } },
                   modelsState === 'loading' ? t('aiModelsLoading') : t('aiModelsLoad')),
               ]),
               React.createElement('p', { className: 'sss-vr-hint' }, t('aiModelHint')),
               React.createElement('input', {
                 className: 'sss-vr-input',
                 type: 'text',
                 ...
                 value: model,
                 list: 'dsh-settings-search-model-options',
                 placeholder: provider === 'ollama' ? 'qwen3:8b'
                ...
                React.createElement('datalist', { id: 'dsh-settings-search-model-options' },
                  aiModels.map((item) => React.createElement('option', { key: item, value: item }, item))),
                modelsMsg
                  ? React.createElement('p', {
                      className: modelsState === 'fail' ? 'sss-vr-note' : 'sss-vr-hint',
                      style: modelsState === 'fail' ? { color: 'var(--dsw-alias-label-danger)' } : undefined,
                    }, modelsMsg)
                  : null,
                provider !== 'ollama' ? React.createElement('div', { className: 'sss-vr-field-head' }, [
                 React.createElement('label', { className: 'sss-vr-label' }, t('aiApiKey')),
               ]) : null,
               provider !== 'ollama' ? React.createElement('input', {
                 ...
                 'data-1p-ignore': 'true',
                 'data-form-type': 'other',
                 onChange: (event) => { setApiKey(event.target.value); resetModelList() },
               }) : null,
              React.createElement('div', { className: 'sss-vr-actions' }, [
            ...
            React.createElement('div', { className: 'sss-vr-field' }, [
              ...
              React.createElement('div', { className: 'sss-vr-actions' }, [
                ...
                React.createElement('a', { className: 'sss-vr-link', href: REPO_URL, target: '_blank', rel: 'noopener noreferrer' },
                  t('updRepo')),
              ]),
              notesState === 'loaded'
            ...
            React.createElement('div', { className: 'sss-vr-field' }, [
              ...
              React.createElement('div', { className: 'sss-vr-actions' }, [
                ...
                React.createElement('a', { className: 'sss-vr-link', href: ISSUES_URL, target: '_blank', rel: 'noopener noreferrer' },
                  t('logIssues')),
